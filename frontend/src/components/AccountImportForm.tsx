...
import type { Account, AccountImportFormValues, ProxyOption } from '../types'
...
interface AccountImportFormProps {
  form: FormInstance<AccountImportFormValues>
...
const AccountImportForm: React.FC<AccountImportFormProps> = ({
  ...
  const handleSubmit = async (values: AccountImportFormValues) => {
    try {
      // 如果还在输入步骤，需要先选择代理地址
      ...
      if (importType === 'privateKey') {
        ...
        const enteredWalletAddress = values.walletAddress?.trim()
        if (enteredWalletAddress) {
          if (enteredWalletAddress !== derivedAddressFromMnemonic) {
      ...
      if (accountsResponse.data.code === 0 && accountsResponse.data.data) {
        const newAccounts: Account[] = accountsResponse.data.data.list || []
        const newAccount = newAccounts.find((acc) => acc.walletAddress === walletAddress)
        if (newAccount) {
