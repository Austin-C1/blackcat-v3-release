...
const ProxySettings: React.FC = () => {
  ...
  const [form] = Form.useForm<ProxySettingsFormValues>()
  ...
  const handleSubmit = async (values: ProxySettingsFormValues) => {
    setLoading(true)
    try {
      const host = values.host?.trim()
      const port = values.port
      if (!host || port === undefined) {
        message.error(t('proxySettings.saveFailed') || 'Failed to save proxy settings')
        return
      }
