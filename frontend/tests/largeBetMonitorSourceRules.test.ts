...
describe('large bet monitor source rules', () => {
  ...
  it('moves copy trading notification filters to robot settings', () => {
    ...
    expect(notificationSettingsSource).toContain('copyTradingLeaderGroups')
    ...
    expect(notificationSettingsSource).toContain('Leader 分组')
    expect(telegramFormSource).not.toContain('监控分类')
    expect(telegramFormSource).not.toContain('消息类型')
  })
})
