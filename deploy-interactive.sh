...
# ========================================
# BlackCat V1 Interactive Deploy Script
# 黑猫V1 交互式一键部署脚本
# ========================================
# Features / 功能：
...
download_docker_compose_file() {
    ...
    error "$(bilingual "未找到本地 docker-compose.prod.yml" "Local docker-compose.prod.yml not found")"
    warn "$(bilingual "请使用当前源码包中的部署文件，或先配置你自己的仓库地址后再下载。" "Use the deployment file from this source package, or set your own repository before downloading.")"
    exit 1
...
generate_env_file() {
    ...
    cat > .env <<EOF
# ========================================
# BlackCat V1 Production Config / 生产环境配置
...
pull_images() {
    ...
    if docker pull blackcat-v1:latest; then
        info "$(bilingual "应用镜像拉取成功" "App image pulled"): blackcat-v1:latest"
    else
...
show_deployment_info() {
    ...
    echo -e "  $(bilingual "更新镜像" "Update"): ${CYAN}docker pull blackcat-v1:latest && docker compose -f docker-compose.prod.yml up -d${NC}"
    echo ""
    ...
    echo -e "  $(bilingual "数据库" "Database"): ${CYAN}blackcat_v1${NC}"
    ...
    warn "  2. $(bilingual "定期备份数据库数据（位于 Docker volume: blackcat-v1_mysql-data）" "Back up DB regularly (Docker volume: blackcat-v1_mysql-data)")"
...
main() {
    ...
    echo ""
    title "========================================="
    title "   $(bilingual "黑猫V1 交互式一键部署脚本" "BlackCat V1 Interactive Deploy")   "
    title "========================================="
    echo ""
