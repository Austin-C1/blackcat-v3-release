...
RUN apt-get update && \
    apt-get install -y nginx curl tzdata jq && \
...
# 创建更新服务相关目录和脚本
RUN mkdir -p /app/backups /var/log/blackcat-v1
...
# 设置目录权限
RUN mkdir -p /var/log/nginx /var/lib/nginx /var/cache/nginx /tmp/nginx && \
    chown -R appuser:appuser /app /var/log/nginx /var/lib/nginx /var/cache/nginx /tmp/nginx && \
    chown -R root:root /usr/share/nginx/html /etc/nginx
