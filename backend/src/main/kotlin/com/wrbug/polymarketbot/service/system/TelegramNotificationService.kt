...
⏰ $timeLabel: <code>$time</code>"""
    ...
    suspend fun sendMonitorPushNotification(
        ...
        val message = notificationTemplateService.renderTemplate(
            "MONITOR_PUSH",
            mapOf(
                "market_title" to escapeHtml(marketTitle),
                "market_link" to marketLink,
                "leader_name" to escapeHtml(leaderName),
                "side" to escapeHtml(sideDisplay),
                "outcome" to escapeHtml(outcome),
                "price" to formatPrice(price),
                "quantity" to formatQuantity(size),
                "amount" to amountDisplay,
                "current_position_summary" to escapeHtml(currentPositionSummary),
                "time" to DateUtils.formatDateTime()
            )
        )
    ...
    suspend fun sendMonitorSameSideNotification(
        ...
        val message = notificationTemplateService.renderTemplate(
            ...
            mapOf(
                ...
                "market_link" to marketLink,
                "outcome" to escapeHtml(outcome),
                "same_side_position_report" to sameSidePositionReport,
    ...
    suspend fun sendMonitorOppositeNotification(
        ...
        val message = notificationTemplateService.renderTemplate(
            ...
            mapOf(
                "market_title" to escapeHtml(marketTitle),
                "market_link" to marketLink,
                "outcome_a" to escapeHtml(outcomeA),
                "side_a_position_report" to sideAPositionReport,
                "outcome_b" to escapeHtml(outcomeB),
                "side_b_position_report" to sideBPositionReport,
                "hedge_position_report" to hedgePositionReport,
                "time" to DateUtils.formatDateTime()
            )
        )
        sendMonitorMessage(message)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val updates = root.get("result")?.mapNotNull { node ->
                    ...
                    TelegramIncomingUpdate(updateId, chatId, text)
                }.orEmpty()
    ...
    suspend fun sendMessage(botToken: String, chatId: String, message: String): Boolean {
        return sendToSingleChat(botToken, chatId, message)
    }
        ...
        return try {
            ...
            val requestBody = objectMapper.writeValueAsString(
                mapOf(
                    ...
                    "text" to message,
                    "parse_mode" to "HTML",
                    "disable_web_page_preview" to false
                )
            )
